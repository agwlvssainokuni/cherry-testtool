...
- `frontend/` — React SPA本体(旧`client/spa`)。ビルド時(`./gradlew build`)に`npm install`→`npm run build`が自動実行され、成果物(`frontend/dist`)が本体jarの`static/`へ組み込まれる
...
cd client/webconsole && ./gradlew bootRun
...
2. 本モジュールを起動する(`cd client/webconsole && ./gradlew bootRun`、`http://localhost:9090`)
