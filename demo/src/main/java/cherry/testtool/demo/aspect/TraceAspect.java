...
/**
 * メソッド呼出しのトレースログを出力するAspect(`lib`の{@code TraceAspect}と同一パターン)。
 * <p>
 * 対象は{@code cherry.testtool.demo}配下(このAspect自身が属する{@code cherry.testtool.demo.aspect}
 * パッケージは対象外)。
 */
@Order(100)
@Aspect
...
public class TraceAspect {
    ...
    @Around("""
            execution(* cherry.testtool.demo..*.*(..)) && !within(cherry.testtool.demo.aspect..*)
            """)
